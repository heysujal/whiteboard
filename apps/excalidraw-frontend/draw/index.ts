    ...
    export async function initDraw(canvas: HTMLCanvasElement, roomId: string, socket: WebSocket) {
        ...
        canvas.addEventListener('mouseup', async (e) => {
            ...
            if(selectedShape === 'rectangle'){
                ...
                const radius = Math.round(Math.sqrt((endX- startX)*(endX - startX) + (endY - startY)*(endY - startY)))/2;
        ...
        canvas.addEventListener('mousemove', function handleMouseDownHold(e) {
            ...
            // Drawing the shape being holded down just for preview without saving it
            if(selectedShape === 'rectangle'){
                ctx.beginPath();
